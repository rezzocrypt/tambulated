...
Интеграция «Списка дел» с Яндекс Календарём работает через CalDAV (`caldav.yandex.ru`) — нужен только логин и пароль приложения (создаётся в `id.yandex.ru/security/app-passwords`), регистрация в консоли разработчиков не требуется.

0. Собрать расширение командой `npm run build`, создастся папка `dist`. Папку назначения можно указать третьим параметром: `npm run build -- <папка>`
...
- **Список дел** — недельная kanban-доска, синхронизированная с Яндекс Календарём через CalDAV: повторяющиеся задачи (разово/ежедневно/по будням/свои дни), drag & drop перенос, отметки выполнения и редактирование.
...
| `npm run build` | Прод-сборка в `dist/` (или `npm run build -- <папка>)` |
